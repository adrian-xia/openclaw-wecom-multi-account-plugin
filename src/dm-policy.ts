...
export async function checkDmPolicy(params: {
  ...
  const pairingApi = core.channel.pairing.readAllowFromStore as unknown as {
    (channel: string, env?: unknown, accountId?: string): Promise<string[]>;
    (params: { channel: string; accountId?: string }): Promise<string[]>;
  };
